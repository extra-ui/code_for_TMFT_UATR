...
class VocabBuilder(object):
    ...
    def count_from_file(path_file, tokenizer=ut._tokenize):
        ...
        df['combined'] = df.apply(lambda row: ' '.join(
            ['The type is', str(row['Type']), 'and', str(row['H_G_D']), 'Chanel_Depth is', str(row['Chanel_Depth']), 'Wind is', str(row['Wind']),
             'Distance is', str(row['Distance'])]), axis=1)
        print(df['combined'][0])
        # 分词
        df['combined'] = df['combined'].apply(tokenizer)

        # 统计词频
        word_count = Counter([tkn for sample in df['combined'].values.tolist() for tkn in sample])

        # 打印原始词汇量
        print('原始词汇量:{}'.format(len(word_count)))
