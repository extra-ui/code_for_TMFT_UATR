...
from .text_embedding import RNN
...
class CAVMAE(nn.Module):
    """ CAV-MAE Model
    """
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=1,
        ...
        self.patch_embed_v = PatchEmbed(img_size, patch_size, 1, embed_dim)

        self.patch_embed_a.num_patches = int(audio_length * 128 / 256)
        self.patch_embed_v.num_patches = int(audio_length * 128 / 256)
    ...
    def initialize_weights(self):
        ...
        pos_embed_v = get_2d_sincos_pos_embed(self.pos_embed_a.shape[-1], 8, int(self.patch_embed_a.num_patches/8), cls_token=False)
        ...
        decoder_pos_embed_v = get_2d_sincos_pos_embed(self.decoder_pos_embed_a.shape[-1], 8, int(self.patch_embed_a.num_patches/8), cls_token=False)
...
class CAVMAEFT(nn.Module):
    ...
    def forward(self, a, v, mode):
        ...
        if mode == 'multimodal':
            ...
            # for blk in self.blocks_u:
            #     x = blk(x)             # (1,708,768)
            # x = self.norm(x)           # (1,708,768)

            # x = x.mean(dim=1)          # (1,768)
            # x = self.mlp_head(x)       # (1,527)
            # x = self.fc(x)
            # a = self.norm(a)
            # a = a.mean(dim=1)
            # v = self.norm(v)
